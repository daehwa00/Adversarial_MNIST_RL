...
class MnistEnv():
    def __init__(self, classification_model, opt, random=True):
        super().__init__()
        self.classification_model = classification_model  # pre-trained CNN model
        self.opt = opt

    def step(self, original_images, action):  # action = [위치, 밝기]
        ...
        arr = []
        center = [
            torch.div(point, 26, rounding_mode="trunc") + 1, point % 26 + 1]
        action = [point, brightness]

        for i in range(self.opt.batch_size):
            changed_image = original_images[i].squeeze(
            ).squeeze().cpu().numpy()  # [28,28]
            # Stamp 찍기
        ...
        with torch.no_grad():
            original_outputs = self.classification_model(
                original_images.to(self.opt.device))
            changed_outputs = self.classification_model(
                changed_images.to(self.opt.device))
